    ...
    <div className="flex flex-col min-h-screen">
      <header className="bg-zinc-900 text-white py-4 px-6 md:px-10 flex items-center justify-between sticky top-0 z-50">
        ...
        <nav className="hidden md:flex items-center space-x-6">
          ...
          {lang === 'kr' ? (
            <Button className="hover:underline" onClick={() => switchLanguage('jp')}>JP</Button>
          ) : (
            <Button className="hover:underline" onClick={() => switchLanguage('kr')}>KR</Button>
          )}
        </nav>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
              <Link className="group" href="https://www.artstation.com/artwork/Vy658X" target="_blank">
                ...
                <p className="text-gray-500">
                  {lang === 'kr' ? '우마무스메 프리티 더비 게임에 등장하는 오구리 캡 이라는 캐릭터입니다.' : 'ウマ娘プリティーダービーゲームに登場するオグリキャップというキャラクターです。'}
                </p>
              </Link>
              <Link className="group" href="https://www.artstation.com/artwork/DvGylG" target="_blank">
                ...
                <p className="text-gray-500">
                  {lang === 'kr' ? '원신에 등장하는 유라 라는 캐릭터입니다.' : '原神に登場するエウラというキャラクターです。'}
                </p>
              </Link>
              <Link className="group" href="#">
                ...
                <p className="text-gray-500">
                  {lang === 'kr' ? '홀로라이브 4기생 토코야미 토와입니다. 아직 제작중입니다.' : 'ホロライブ4期生常闇トワです。まだ制作中です。'}
                </p>
              </Link>
            </div>
          </div>
        ...
        <section className="bg-zinc-800 text-white py-20 px-6 md:px-10" id="about">
          <div className="max-w-5xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-10">
            <div className="space-y-6">
              ...
              <p className="text-lg md:text-xl">
                {lang === 'kr' ? '대한민국 국적에 02년생 남자 이며, 이름은 김동식입니다. 주로 3D 캐릭터 모델링과 3D 캐릭터 애니메이션을 작업합니다.' : '韓国国籍の02年生まれの男性で、名前はキム・ドンシクです。主に3Dキャラクターモデリングと3Dキャラクターアニメーションを制作します。'}
              </p>
            </div>
            <div className="relative h-[400px] md:h-[500px] overflow-hidden rounded-lg shadow-lg">
